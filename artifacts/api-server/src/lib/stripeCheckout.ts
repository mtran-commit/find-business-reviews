...
/** Find the price behind the legacy Payment Link so per-customer checkouts
 * charge exactly the same amount. Returns null when not found (e.g. test-mode
 * key while the link is live-mode). */
 ...
 * 1. the price behind the legacy Payment Link (exact parity),
 * 2. a previously created price by lookup key,
