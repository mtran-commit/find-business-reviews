...
NEVER show a wrong or generic logo (WordPress/Wix/Squarespace/Shopify/favicon/platform icons). There is currently **no confident logo source**, so the backend sets `logoConfidence = "low"` everywhere and the UI falls back to navy initials (business photo still shows). To re-enable logos, add a confident source and set `logoConfidence = "high"` only for confidently-matched marks. Business photos (`imageUrl`, the Google Maps thumbnail) are always shown.
