...
- **Phased search (iOS-safe)**: iPhones hard-kill any web request at ~60s, so search is two-phase. Phase 1 `GET /api/search-business?phase=core` (~25s) returns Google data + logo + nearby with the 5 slow platforms `pending`; the page renders immediately with pulsing "Checking…" rows. Phase 2 fires 5 parallel `GET /api/search-business-platform?platform=...&query=...&name=...&address=...` calls; each result updates its row and recomputes the Trust Score via targeted DOM updates (`refreshLiveResults` — never a full re-render, so photos/listeners survive). A `searchToken` drops late responses from an older search. The platform endpoint has a 50s server cap → `"Lookup timed out"` note. `phase` omitted/`full` keeps the old single-call behaviour (used by report generation via `fetchBusinessReviews`, which composes `fetchBusinessCore` + `resolvePlatformRating`).
...
### Wowlette offers (partner app)
`lib/wowlette.ts` + `GET /api/wowlette-offers?name=...` proxy the user's other Replit app Wowlette (`GET <WOWLETTE_BASE_URL>/api/public/offers/search?name=...`, no auth). Base URL from the `WOWLETTE_BASE_URL` env var (https added if missing, origin-validated); 8s timeout, zod-validated (offers require http(s) `addToWalletUrl`), businesses with 0 offers filtered out, 10-min in-memory cache. The route ALWAYS returns 200 `{available, businesses}` — any failure/missing config degrades to `{available:false}` so review search is never slowed or broken. Frontend: `loadWowletteOffers` fires in parallel after results render (token-guarded, silent on failure) and fills the `.wowlette-slot` div between the featured card and similar list with purple-bordered offer cards ("Add offer to Wowlette Wallet" buttons via `safeOpen`, all text `escapeHtml`ed). Hidden entirely when no offers. The legacy demo `data.offer` is untouched. Paid reports: `fetchWowletteOffers` also returns `lookupSucceeded` (true only when configured AND the HTTP lookup returned a valid payload — even with 0 offers); this is persisted as `wowletteAvailable` on `BusinessReport` (defaults false for old reports). When the lookup succeeded but there are no offers, PDF/HTML reports show a short "publish offers on Wowlette" invite; when unconfigured/failed, nothing is shown.
